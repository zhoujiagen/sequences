...
public class TPFileSystemProvider extends FileSystemProvider {
  ...
  public TPFileSystemProvider() {
    try {
      TPFileSystem fileSystem =
          newFileSystem(URI.create(TPFSConfiguration.FS_SCHEMA_PATH),
  ...
  public FileStore getFileStore(Path path) throws IOException {
    return Lists.newArrayList(TPFSCB.I().getFileSystem().getFileStores()).get(
      (int) ((path.normalize().hashCode() % TPFSConfiguration.storeSize)));
  }
  ...
  public FileSystem getFileSystem(URI uri) {
    ...
    Preconditions.checkArgument(getScheme().equals(schema), "not support schema " + schema
        + ", only " + getScheme() + " supported");
